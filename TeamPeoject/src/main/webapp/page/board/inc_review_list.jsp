				...
				<li>
					<div class="review_inner flex col">
						<div class="con_wrap flex vc space">			
							...
							<!-- 본인 글에만 보이게 -->	
							<c:if test="${not empty loginUser && loginUser.no == board.user_no}">
								<div class="page_btn">				
							        <form action="${pageContext.request.contextPath}/board/delete" method="post" class="form-delete">
							            <input type="hidden" name="no" value="${board.no}">						
							            <input type="hidden" name="place_no" value="${place.no}">						
										<button onclick="layerOpen('${pageContext.request.contextPath}/board/edit?no=${board.no}')" type="button" class="input_st s2 c1 re">수정</button>
						...
						<div class="con_wrap flex vc space">			
							<div class="rateStar_wrap flex di vc">
								<c:forEach var="s" begin="1" end="${board.rating}">
									<span class="mti mtifill material-symbols-outlined star full">star</span>
								</c:forEach>
								<c:forEach var="s" begin="1" end="${5 - board.rating}">
									<span class="mti material-symbols-outlined star">star</span>
								</c:forEach>
